...
class ComplianceAgent:
	...
	def evaluate(self, strategy: str, retrieved_rules: Iterable[RetrievedRegulation], risk_profile: dict[str, object] | None = None) -> StrategyCheckResponse:
	...
	@staticmethod
	def _build_citations(retrieved_rules: list[RetrievedRegulation], limit: int) -> list[Citation]:
		citations: list[Citation] = []
	...
	@staticmethod
	def _is_ambiguous(strategy: str, retrieved_rules: list[RetrievedRegulation]) -> bool:
		...
		if not retrieved_rules:
			return True
		return max(rule.confidence for rule in retrieved_rules) < 0.3
...
class ComplianceAgent:
	# Evaluate a strategy against retrieved regulations and basic risk signals.
	def evaluate(self, strategy: str, retrieved_rules: Iterable[dict[str, Any]], risk_profile: dict[str, object] | None = None) -> StrategyCheckResponse:
		retrieved_rules = list(retrieved_rules)
		risk_profile = risk_profile or {}
		strategy_lower = strategy.lower()
		strategy_id = f"strategy-{uuid4().hex[:12]}"
		...
		if final_decision == "APPROVED" and bool(risk_profile.get("short_selling")):
			if any("short" in str(rule.get("text", "")).lower() for rule in retrieved_rules):
				final_decision = "NEEDS_REVIEW"
				reason = "NEEDS_REVIEW: Short-selling may require jurisdiction-specific checks and position-limit review."
				risk_flags.append("short_selling_review")
				recommendations.append("Verify short-selling constraints before execution.")
	...
	# Convert retrieved rule dictionaries into the citation objects returned by the API.
	@staticmethod
	def _build_citations(retrieved_rules: list[dict[str, Any]], limit: int) -> list[Citation]:
		citations: list[Citation] = []
		for rule in retrieved_rules[:limit]:
			# Dynamically pick the available scoring systems
			score = float(rule.get("rerank_score", rule.get("retrieval_score", 0.5)))
			citations.append(
				Citation(
					source_id=str(rule.get("source_id", "")),
					source_name=str(rule.get("source_name", "")),
					regulation_title=str(rule.get("regulation_title", "")),
					jurisdiction=str(rule.get("jurisdiction", "")),
					effective_date=str(rule.get("effective_date", "")),
					section=str(rule.get("section", "")),
					chunk_id=str(rule.get("chunk_id", "")),
					confidence=round(score, 4),
					excerpt=str(rule.get("text", ""))[:240],
				)
			)
		return citations
	...
	# Decide whether the strategy is too vague to score confidently.
	@staticmethod
	def _is_ambiguous(strategy: str, retrieved_rules: list[dict[str, Any]]) -> bool:
		ambiguous_markers = ["maybe", "could", "potentially", "unclear", "borderline", "experimental", "highly leveraged"]
		if any(marker in strategy for marker in ambiguous_markers):
			return True
		if not retrieved_rules:
		...
		for rule in retrieved_rules:
			score = float(rule.get("rerank_score", rule.get("retrieval_score", 0.0)))
			if score > max_score:
				max_score = score
